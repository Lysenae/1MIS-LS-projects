 ...
 *
 * Compile: gcc -o send_arp send_arp.c
 * Execute: ./send_arp 192.168.1.39
 *
 **/
#include <stdio.h>
#include <stdlib.h>
...
/* 
 * For function inet_ntop and inet_pton
 * 
 */
#include <arpa/inet.h>
...
int main(int argc, char **argv)
{
	...
	u_char *mac;
	char recv_buf[120], rep_addr[INET_ADDRSTRLEN];
	...
	if(get_ifi(IF_INTERFACE, mac, ETH_ALEN, &lc_addr, INLEN)) {
	...
	inet_pton(AF_INET, argv[1], req.ea.arp_tpa);
	...
	while(1) {
		...
		if( ntohs(*(__be16 *)(recv_buf + 20))==2 && !memcmp(req.ea.arp_tpa, recv_buf + 28, 4) ) {
			...
			printf("\t%s\n", inet_ntop(AF_INET, (struct in_addr *)(recv_buf + 28), rep_addr, sizeof(rep_addr)));
			break;
		}
	}
